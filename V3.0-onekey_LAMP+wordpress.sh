...
yum install -y wget
yum install -y cmake
yum install -y gcc gcc-c++ autoconf libtool expat-devel

#安装MySQL依赖包
yum install -y readline-devel zlib-devel openssl-devel install libaio-devel boost-devel
...
#定义Apr安装路径
HA_URL=http://mirrors.hust.edu.cn/apache//apr/apr-1.6.5.tar.gz
HA_FILES=apr-1.6.5.tar.gz
HA_FILES_DIR=apr-1.6.5
HA_PREFIX=/usr/local/apr

#定义apr-util安装路径
HU_URL=http://mirrors.hust.edu.cn/apache//apr/apr-util-1.6.1.tar.gz
...
#定义Apache安装路径
H_URL=http://mirrors.hust.edu.cn/apache//httpd/httpd-2.4.37.tar.gz
...
#################################################
#临时添加swep分区，解决因内存不足导致编译失败问题

echo "正在创建临时swap分区，防止因内存不足编译失败"
...
cd $S_DIR
tar -zxvf $PM_FILES
mv $PM_FILES_DIR/*  $H_PREFIX/htdocs/phpmyadmin

#安装wordpress
mkdir -p $H_PREFIX/htdocs/wordpress
cd $S_DIR
tar -zxvf $W_FILES
mv $W_FILES_DIR/* $H_PREFIX/htdocs/wordpress
...
#初始化MySQL数据库
cd $M_PREFIX
./scripts/mysql_install_db --user=mysql --datadir=$M_PREFIX/data
...
$M_PREFIX/bin/mysql -h${HOSTNAME} -P${PORT} -u${USERNAME} -p${PASSWORD} -e"${create_db_sql}"
...
#复制PHP配置文件
cp $S_DIR/$P_FILES_DIR/php.ini-production /etc/php.ini
